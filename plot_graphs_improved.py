...
def parse_methyl_sensitivity(sensitivity_file):
	"""
	Function to parse the methylation sensitivity from the input file. The input file contains 3 columns, tab separated, with the family of the TF in the first column, its name in the second one and the sensitivity (float) in the last one. The file should have a first line with the names of the columns. 
	It returns a dictionary with the TF references as keys (family/name). The values of the dictionary are lists with 2 elements, the first one for the sensitivity of the TF, the other for a score (AUPRC and AUROC) retrieved later. 
	"""
	values = {}
	...
	with open(sensitivity_file, 'r') as sensitivity:
		...
		for line in sensitivity:
			...
			values[TF] = [float(features[2]), 0.0]
			
	return values


def parse_scores(score_file, methyl_sensitivity_file, from_str, method_str):
	"""
	Function to parse the AUPRC or AUROC scores in the input file. Each scores' file has 5 tab separated columns: family of the TF, name of the TF, initial motif (methylated or not), method + methylation state used for the predictions and the score obtained. See the function "parse_methyl_sensitivity" for more information about the methylation sensitivity file. Only the TFs for which we have information about their methylation sensitivity are considered.
	This function returns a list of tuples with the following structure: (TF_family/TF_name, [methylation sensitivity, score]). 	That list is sorted by increasing methylation sensitivity.
	"""
	...
	with open(score_file, 'r') as data:
		...
		for line in data:
			...
			if TF in scores:										#Only the TFs with methylation sensitivity data
				if features[2] == from_str and features[3] == method_str:
	...
	#Sort by methylation sensitivity, then by name of TF
	scores_sorted = sorted(final_scores.iteritems(), key=lambda x: (x[1][0], x[0]))
...
def classify_by_sensitivity(scores):
	"""
	This function takes a scores' dictionnary containing also the methylation sensitivity and creates a new dictionary with 3 lists to classify by methylation sensitivity: methyl_plus for the TFs that bind preferentially when cytosines are methylated, methyl_minus for the TFs that bind preferentially when cytosines are NOT methylated and non_sensitive when the TF binding is not affected by methylation.
	The function returns that dictionary.
	"""
	
	scores_classified = {'methyl_plus':[], 'non_sensitive':[], 'methyl_minus':[]}
#	labels = []
	
	for TF in scores:
#		labels.append(TF[0])
		if TF[1][0] >= 1.0:
			scores_classified['methyl_plus'].append(TF[1][1])
			scores_classified['non_sensitive'].append(np.nan)
			scores_classified['methyl_minus'].append(np.nan)
		
		elif TF[1][0] <= -1.0:
			scores_classified['methyl_minus'].append(TF[1][1])
			scores_classified['non_sensitive'].append(np.nan)
			scores_classified['methyl_plus'].append(np.nan)
			
		else:
			scores_classified['non_sensitive'].append(TF[1][1])
			scores_classified['methyl_plus'].append(np.nan)
			scores_classified['methyl_minus'].append(np.nan)	
	
	return scores_classified
...
def generate_graphs(AUPRC_file, AUROC_file, methyl_sensitivity_file):
	...
	type_list = ["AUPRC", "AUROC"]
	from_list = ["from_methyl", "from_non_methyl"]
	method_list = ["wm_FIMO_wm_shapes", "wo_FIMO_wm_shapes", "wm_FIMO_wo_shapes", "wo_FIMO_wo_shapes", "wm_FIMO", "wo_FIMO"]
	...
	for type_str in type_list:
		if type_str == "AUPRC":
			score_file = AUPRC_file
		else:
			score_file = AUROC_file
			
		for from_str in from_list:
			for method_str in method_list:
				key = "{0}_{1}_{2}".format(type_str, from_str, method_str)
				scores[key] = parse_scores(score_file, methyl_sensitivity_file, from_str, method_str)
	
	for key_el in scores:
		scores_classified[key_el] = classify_by_sensitivity(scores[key_el])
